# v551app-builder — Mobile CI/CD

GitHub Actions pipelines that build the **v551app** mobile app (iOS + Android) and ship it to Diawi / TestFlight / Google Drive, using a **reusable-workflows + composite-actions** architecture.
...
│   ├── prepare-mobile-build/    # Ruby 3.2.6 + Fastlane + gem cache
...
| `xcode_version` | `26.3` | `16.4` or `26.3` |
...
| `reusable-ios-adhoc.yml` | `macos-26` / `macos-15` | 30m | `ios adhoc` | .ipa → Diawi |
...
| `prepare-mobile-build` | – | `ruby/setup-ruby` **3.2.6**, caches gems, `bundle install` in `real-code/fastlane` |
...
|----------|-------|-------------|
| `PROJECT_NAME` | `v551app` | Shown in notifications |
...
| CocoaPods | `<os>-pods-<Podfile.lock>` (`ios/Pods`, `~/.cocoapods/repos`) | iOS |
| Xcode compilation cache (CAS) | `xcode-compcache-<xcode>-<run_id>` | iOS, **Xcode 26 only** |
| Yarn | `yarn-<yarn.lock>` | iOS TestFlight |

> **Xcode CAS** (LLVM content-addressed) only engages on `macos-26` when the app's `gym` passes `COMPILATION_CACHE_ENABLE_CACHING=YES`. It replaces ccache (dead on Xcode 26.3). The `<run_id>` in the key + `restore-keys` prefix = rolling "newest snapshot per Xcode version".
...
curl -X POST \
  ...
  https://api.github.com/repos/bssteam02/v551app-builder/actions/workflows/build-aab.yml/dispatches \
...
These are baked into the workflows for this app and must be updated when cloning this builder for a different app (they are cross-project copy-paste leftovers, some inconsistent):

| Where | Value | Note |
|-------|-------|------|
| `reusable-build-aab.yml` | Android keystore filename | — |
| `reusable-build-apk.yml` | Android keystore filename | ⚠️ currently differs from the AAB one |
